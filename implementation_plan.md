...
2026-09-19 核心收尾：按用户新增$1及“只验证核心功能”要求，实际22次合成任务模型请求合计峰时估算$0.0466506，无本批未知用量，现已停止付费验证。修复格式化减法误判和表格跨列串行误判，共用交付规则去重；最后研究/项目各3次调用完成正文及核对，但仍有模型未检出的口径/责任推断，不能宣布完整内容达标。285项定向回归及Core类型/lint/构建通过，本机新版已恢复且数据配置不变。主项目与三个独立小工具均已交付GitHub，不再扩展产品或重复全面验收；本轮事实及保留限制见[核心收尾](./docs/release-readiness.md#限额核心复测与收尾2026-09-19)。
