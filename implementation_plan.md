...
2026-09-19 核心收尾：用户新增$1授权内，22次合成任务请求及随后7次有界诊断合计保守估算$0.09802386，无本批未知用量，已停止付费测试。格式化算术、表格跨列/表头及职责范围检查已补修；最新修复让有材料引用的有效内容失败不被另一条无效核对记录挡住，仍最多修订一次。131项相关回归、Core类型/lint/构建通过，本机新版已恢复且数据配置不变。模型仍可能漏判口径/责任推断，Pro单次诊断也存在算术字段错误，不能宣布完整内容达标；默认Flash不变。主项目与三个独立小工具均已交付GitHub，不扩展产品或重复全面验收；事实及保留限制见[核心收尾](./docs/release-readiness.md#限额核心复测与收尾2026-09-19)。
