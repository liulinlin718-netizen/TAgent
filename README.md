...
2026-09-18 追加验收：已用独立授权内14次模型调用，峰时无缓存估算$0.0295077，取得完整办公核对回执并跑通真实页面发送、SSE、保存刷新和工作流拖拽。人工发现的风险标题过强已补本地检查，247项相关回归通过；不把模型自评或已知规则通过等同整份报告质量合格。费用、截图与具体剩余问题见 [追加验收记录](./docs/release-readiness.md#追加授权与真实页面回查2026-09-18)。下方各轮数字为历史记录。

2026-09-18 后续开发已补必要验证：办公材料/风险边界与新增检查通过236项定向回归、Core类型/lint、Core/Server构建及18场景隔离接口检查；本机已加载新版后端，保留原数据与前端构建，无外部模型/搜索调用。真实报告内容质量仍待复核，当前实现、已交付的三个独立小工具与剩余缺口见 [目标交付核对](./docs/release-readiness.md#当前目标交付核对2026-09-18)。下面1487项是此前全量记录，不代表本次重跑全仓。

2026-09-18 本地验证更新：只读探索、运行状态与周期待办、执行快照分支和治理依据回查已通过本轮必要离线检查，完成生产构建并启动 G 盘本机版本。完整回归101文件1487项、40项脚本自检及1440/390/320页面检查通过；没有新增付费模型或搜索请求，不把模拟结果当作真实报告质量证明。范围和证据见 [运行、探索与快照](./docs/runtime-explore-snapshots.md)。

TAgent 现在已经不是单纯的概念文档，仓库中已有一套可运行的 TypeScript monorepo 原型：

本机交付范围与最新验收见 [当前验收记录](./docs/release-readiness.md#最新授权与验收2026-09-15)。当前默认 DeepSeek 模型为 `deepseek-flash`，显式配置不覆盖；核对失败保留原稿，不把模型自评通过等同独立事实核查。研究/项目回复的部分语义判断仍在优化，暂不宣称所有办公任务已达生产质量。
