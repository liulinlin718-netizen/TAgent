...
2026-09-19：G盘本机版本已更新并运行，主项目与三个独立开源工具已交付GitHub。最新修复允许有实际材料引用的有效内容失败进入原有一次修订，避免被另一条无效核对记录阻断；131项相关回归、Core类型/lint及构建通过，原数据和模型配置不变。功能交付范围见 [目标核对](./docs/release-readiness.md#当前目标交付核对2026-09-18)。

最新追加$1测试授权内共29次调用，保守估算$0.09802386，无本批未知用量，已停止付费测试。模型核对仍可能漏判口径与责任范围，不等于事实认证；默认Flash未切换，不以更强模型的单次对照宣称整份内容达标。完整证据、费用和保留限制见 [核心收尾](./docs/release-readiness.md#限额核心复测与收尾2026-09-19)，其中各日期的测试数量只代表对应版本。
