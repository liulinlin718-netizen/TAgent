...
本轮已按用户确认收尾为**需人工复核的本机试用版**。应用代码 `cc9ab27` 已通过 [Windows / Ubuntu 完整 CI](https://github.com/liulinlin718-netizen/TAgent/actions/runs/35388634291)，G盘前后端正常运行；主项目及三个独立开源工具已交付。报告内容质量和未结算费用仍如实保留，不再继续付费测试，详见 [版本收尾](./docs/release-readiness.md#本机试用版收尾2026-09-19)。

2026-09-19：G盘本机版本已更新并运行，主项目与三个独立开源工具已交付GitHub。新增可选的独立办公核对模型配置，生成和工具执行仍用原模型，核对与最多一次修订共享任务预算。管理页显示实际配置，未自动启用更贵模型；239项相关回归、隔离接口、类型/lint/构建及桌面/窄屏显示检查通过。功能交付范围见 [目标核对](./docs/release-readiness.md#当前目标交付核对2026-09-18)。

最新追加$1测试授权内共37次派发，已知保守估算$0.11293266，另保留1次核对超时的$0.16078128未知用量预留；本地没有在途请求，不自动重试。最后一次项目任务保留了排期原稿，但较强模型核对超时，不能记为完整内容通过。默认Flash未切换，不把模型自评当作事实认证。证据与限制见 [独立核对配置与收尾](./docs/release-readiness.md#独立核对配置与收尾2026-09-19)，各日期测试数量只代表对应版本。
